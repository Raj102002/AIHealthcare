      ...
      pieces.forEach((piece, i) => {
        ...
        chunks.push({
          ...
          metadata: {
            ...
            section_path: `${doc.title} > ${section.heading}`,
...
async function main() {
  ...
  for (const raw of rawChunks) {
    ...
    const retrievedAt = prior && prior.content_hash === contentHash ? prior.retrieved_at : nowIso;
    ...
    chunks.push({
      id: raw.id,
      text: raw.text,
      metadata: { ...raw.metadata, retrieved_at: retrievedAt, content_hash: contentHash },
    });
  }
    ...
    try {
      ...
      for (let i = 0; i < toUpsert.length; i += BATCH) {
        ...
        for (const c of batch) {
          confirmed[c.id] = { content_hash: c.metadata.content_hash, retrieved_at: c.metadata.retrieved_at };
        }
  ...
  for (const c of chunks) {
    ...
    if (!toUpsert.some((u) => u.id === c.id)) {
      newManifest[c.id] = { content_hash: c.metadata.content_hash, retrieved_at: c.metadata.retrieved_at };
    }
  }
  for (const id of staleIds) {
